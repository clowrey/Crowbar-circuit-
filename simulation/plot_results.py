...
def load_ngspice_data(filepath):
    ...
    return {
        'time':     raw[:, 0],
        'v_rect':   raw[:, 1],   # crowbar/rectifier node (pre-blocking SCR)
        'v_out':    raw[:, 3],   # protected output node (post-blocking SCR)
        'v_ref':    raw[:, 5],
        'v_gate':   raw[:, 7],
        'i_supply': raw[:, 9],
        'v_local':  raw[:, 11],
...
def main():
    ...
    rect_collapse = find_crossing(d['v_rect'], 100, 'falling')
    tl431_trip = find_crossing(d['v_ref'], 2.49, 'rising')
    peak_idx = np.argmax(np.abs(d['i_supply']))
    peak_i = d['i_supply'][peak_idx]

    print("=" * 60)
    print("  CROWBAR CIRCUIT SIMULATION RESULTS")
    print("=" * 60)
    print(f"  Rect bus range:   {d['v_rect'].min():.1f}V – {d['v_rect'].max():.1f}V")
    print(f"  Output range:     {d['v_out'].min():.1f}V – {d['v_out'].max():.1f}V")
    print(f"  Output @ end:     {d['v_out'][-1]:.1f}V")
    print(f"  Ref node range:   {d['v_ref'].min():.4f}V – {d['v_ref'].max():.4f}V")
    print(f"  Gate peak:        {d['v_gate'].max():.3f}V")
    print(f"  Peak surge I:     {np.abs(peak_i):.0f}A")
    print(f"  Local supply:     {d['v_local'].max():.1f}V")
    if scr_fire is not None:
        print(f"  SCR fires at:     {t_ms[scr_fire]:.2f} ms  (V_out = {d['v_out'][scr_fire]:.1f}V)")
    if rect_collapse is not None:
        print(f"  Rect bus collapses at: {t_ms[rect_collapse]:.2f} ms")
    print("=" * 60)
    ...
    C = {
        'rect': '#1565C0', 'out': '#00897B', 'ref': '#2E7D32', 'gate': '#7B1FA2',
    ...
    fire_t = t_ms[scr_fire] if scr_fire is not None else None
    ...
    # ---- Panel 1: Rectifier bus vs protected output ----
    ax = axes[0]
    ax.fill_between(t_ms, 0, d['v_rect'], alpha=0.05, color=C['rect'])
    ax.plot(t_ms, d['v_rect'], color=C['rect'], linewidth=2, label='V(Rectifier/Crowbar Bus)')
    ax.plot(t_ms, d['v_out'], color=C['out'], linewidth=1.8, label='V(Protected Output)')
    ...
    ax.set_title('Crowbar Bus Collapse with Output Hold-Up', fontsize=12, fontweight='bold', pad=8)
    ax.legend(loc='upper left', fontsize=9, framealpha=0.9)
    ax.set_ylim(-30, 560)

    if scr_fire is not None:
        ax.annotate(
            f'CROWBAR FIRES\nVout={d["v_out"][scr_fire]:.1f}V @ {t_ms[scr_fire]:.1f}ms',
            xy=(t_ms[scr_fire], d['v_rect'][scr_fire]),
        ...
        ax.text(700, 500, 'RECT BUS\nCOLLAPSED', fontsize=9, color=C['current'],
                ha='center', va='center', alpha=0.6, fontweight='bold')
        ax.text(700, 330, 'OUTPUT HELD\nBY C_OUT + SCR2 BLOCKING', fontsize=9, color=C['out'],
    ...
    ax.plot(t_ms, d['i_supply'], color=C['current'], linewidth=1.5, label='I(Rectifier → Crowbar Bus)')
    ax.set_ylabel('Current (A)', fontsize=11)
    ax.set_xlabel('Time (ms)', fontsize=12)
    ax.set_title('Rectifier Feed Current', fontsize=11, fontweight='bold', pad=6)
    ax.legend(loc='upper left', fontsize=9, framealpha=0.9)

    if scr_fire is not None:
        ax.annotate(f'Peak surge: {np.abs(peak_i):.0f}A',
    ...
    if fire_t is not None:
        ...
        ax2 = axes2[0]
        ax2.plot(t_ms[mask], d['v_rect'][mask], color=C['rect'], linewidth=2, label='V(Rectifier Bus)')
        ax2.plot(t_ms[mask], d['v_out'][mask], color=C['out'], linewidth=1.7, label='V(Output)')
        ax2.axhline(y=450, color=C['thresh'], linestyle='--', alpha=0.8)
        ax2.set_ylabel('Voltage (V)')
        ax2.set_title('Rectifier Bus Collapse vs Output Hold-Up', fontweight='bold')
        ...
        ax2.set_ylabel('I(Rectifier) (A)')
        ax2.set_xlabel('Time (ms)')
        ax2.set_title('Rectifier Feed Surge Current', fontweight='bold')
        ax2.grid(True, alpha=0.3)
